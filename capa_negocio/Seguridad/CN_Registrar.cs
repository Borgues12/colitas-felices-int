...
namespace capa_negocio.Seguridad
{
    public class CN_Registrar
    {
        ...
        public async Task<bool> CambiarEmailRegistro(string antiguoEmail, string nuevoEmail)
        {
            // Validaciones de negocio antes de ir a la BD
            if (string.IsNullOrWhiteSpace(antiguoEmail) || string.IsNullOrWhiteSpace(nuevoEmail))
                return false;
            if (antiguoEmail.Trim().ToLower() == nuevoEmail.Trim().ToLower())
                return false;
            return await cdRegistro.CambiarEmailRegistro(antiguoEmail, nuevoEmail);
        }
